<template>
    <aside id="kendirimolsaidi" :style="sideBarActive ? 'display:block' : 'display:none'" class="u-sidebar u-sidebar__lg" >
                         ...
                         <div class="u-sidebar__content u-header-sidebar__content">
                            <div :class="[loadik ? 'loadik' : '']"></div>
                             <form v-if="!store.state.user.isLoggedIn || setPassword" @keydown.enter.prevent class="js-validate">
                                 <!-- Login -->
                                 <div id="login" :style="current === 1 ? 'display:block' : 'display:none'">
                                     <!-- Title -->
                                     ...
                                     <div v-if="!forgotting && !setPassword" class="text-center mb-4">
                                         ...
                                         <a class="js-animation-link small text-dark" href="javascript:;" @click.prevent="current = 2">{{store.state.user.language.auth.login.sign_up}}
                                         </a>
                                     </div>
                                 </div>

                                 <!-- Signup -->
                                 <div id="signup" :style="current === 2 ? 'display:block' : 'display:none'">
                                     <!-- Title -->
                                     <header class="text-center mb-7">
                                     ...
                                     <div v-if="verifyng" class="form-group">
                                         <div class="js-form-message js-focus-state">
                                             ...
                                             <div class="input-group" :style="validateToken ? 'box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)' : ''">
                                                 ...
                                                 <input v-model="registerForm.v_token" type="password" class="form-control" :placeholder="store.state.user.language.auth.verification.placeholder" :style="validateToken ? 'border-color:red;' : ''">
                                     ...
                                     <div v-show="verifyng" class="mb-2">
                                         <button type="button" @click.prevent="verificate" class="btn btn-block btn-sm btn-primary transition-3d-hover">{{store.state.user.language.auth.verification.verificate}}</button>
                                     </div>
                                     <div v-if="!verifyng" class="form-group">
                                         <div class="js-form-message js-focus-state">
                                    ...
                                    <div v-if="!verifyng" class="form-group">
                                     ...
                                     <div v-if="!verifyng" class="text-center mb-4">
                                         ...
                                         <a class="js-animation-link small text-dark" href="javascript:;" @click.prevent="current = 1">{{store.state.user.language.auth.register.sign_up}}
                                         </a>
                                     </div>
...
watch(()=>store.state.sidebarActive,()=>{
    sideBarActive.value = store.state.sidebarActive
})
...
onMounted(()=>{
    ...
    $(document).on('click', function(event) {
      var $element = $('#kendirimolsaidi'); 
      var $ooy = $('#sidebarNavToggler'); 
      var $doy = $('.thoseWhoShallSuffer'); 
      if (!$element.is(event.target) && !$ooy.is(event.target) && !$doy.is(event.target) && !$element.has(event.target).length && !$ooy.has(event.target).length && !$doy.has(event.target).length && store.state.sidebarActive) {
            hideSidebar()
      }
});
})
...
const register = async() => {
    ...
    if(validateName() && validateEmail() && validatePassword() && validatePasswordConfirmation()){
        ...
        await axios.post(api+'sendverification',registerForm).then(()=>{
            verifyng.value = true
            loadik.value = false
        }).catch((error)=>{
            loadik.value = false
...
const iforgor = async() => {
    loadik.value = true
    await axios.post(api+'forgotpassword',loginForm).then(()=>{
            loadik.value = false
